...
class EMC2101Component : public Component, public i2c::I2CDevice {
 public:
  EMC2101Component() = default;

  /// Check i2c availability and setup masks
  void setup() override;

  uint8_t readByte(uint8_t register);
  bool readByte(uint8_t register, uint8_t *val);
  bool writeByte(uint8_t register, uint8_t val);
  uint32_t readBit(uint8_t register, uint8_t _bits, uint8_t _shift);
  bool writeBit(uint8_t register, uint8_t _bits, uint8_t _shift, uint32_t data);
